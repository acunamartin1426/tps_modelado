...
Las ecuaciones del modelo SIR (_susceptibles_, _infectados_, _recuperados_) son:

$$\left\{\begin{array}{rcl}
	\dot{S} &=& -\beta \frac{SI}{N} \\ 
	\dot{I} &=& \beta \frac{SI}{N} -\sigma I \\
	\dot{R} &=& \sigma I
\end{array},\right.$$

donde $N$ es la cantidad total de individuos de la población. Al haber normalizado los datos, para nosotros $N=1$. Los parámetros $\beta$ y $\sigma$ son las tasas de transmisión y de recuperación. Sus inversos también admiten una interpretación sencilla. $\beta^{-1}$ pueden pensarse como el tiempo típico entre contagios y $\sigma^{-1}$ como el tiempo medio de recuperación. Un parámetro importante es $R_0 = \frac{\beta}{\sigma} = \frac{\sigma^{-1}}{\beta^{-1}}$, conocido como la _tasa básica de reproducción_ que indica la cantidad de nuevos infectados producida por un único infectado.
...
El modelo SEIR es similar al SIR pero agrega un estadío intermedio de personas _expuestas_. Una persona expuesta es alguien que se ha contagiado el virus, pero aún no expresa síntomas y no contagia a otros. Las ecuaciones del modelo son:

$$\left\{\begin{array}{rcl}
	\dot{S} &=& -\beta \frac{SI}{N} \\ 
	\dot{E} &=& \beta \frac{SI}{N} - \gamma E \\
...
md""" #### Cotas
Es fácil observar que el optimizador es muy sensible a los valores iniciales de los parámetros y a las cotas que se impongan sobre ellos (`lb` y `ub`). Comenzar fijando cotas que consideren razonables, teniendo en cuenta que: 
+ β⁻¹ = tiempo medio entre contagios,
+ σ⁻¹ = tiempo medio de recuperación,
+ γ⁻¹ = tiempo medio de incubación,
+ δ⁻¹ = tiempo medio de pérdida de inmunidad.
+ Dado que los datos procesados se cuentan por semana, los inversos de los parámetros dan tiempos _en semanas_. 
Probar distintos valores iniciales dentro de las restricciones. En caso de no conseguir ajustes satisfactorios, ir modificando las cotas hasta conseguir ajustes que luzcan correctos (ver punto siguiente).
"""


# ╔═╡ 4e0aaf37-4150-4526-b0fe-707bd8d9602b
md"""#### Comparación:
Implementar una función que, dado un conjunto ajustado de parámetros de un dado modelo, resuelva el problema con los datos iniciales y los parámetros correspondientes y grafique $\beta SI$ junto con los datos. Además, evaluar la función de `costo` en la solución obtenida. 
Para cada uno de los modelos analizar: 
+ Si el gráfico parece adecuado (esto debería conseguirse en todos los casos).
+ ¿Cuál permite obtener un menor error en el ajuste? 
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "07bbbacd655d6ac1c063f3d496352bd75911bacb"
...
git-tree-sha1 = "bf644c4b05ea7eb6fee7f25ebd45ff95ae960470"
